...
async fn handle_client(mut client_stream: TcpStream) -> Result<(), Error> {
    ...
    match client_stream.read(&mut buffer).await {
        Ok(0) => {
            println!("Conexão encerrada abruptamente (EOF). Tentando manter aberta...");
            return Ok(());
        }
        Ok(_) => {
            client_stream
    ...
    if server_connect.is_err() {
        println!("Erro ao conectar ao proxy.");
        return Ok(());
    }
    ...
    loop {
        let bytes_read = {
            ...
            match read_guard.read(&mut buffer).await {
                Ok(0) => {
                    println!("EOF detectado. Enviando dados falsos para manter conexão...");
                    let mut write_guard = write_stream.lock().await;
                    write_guard.write_all(b"PING\r\n").await?;
                    continue;
                }
                Ok(n) => n,
                Err(e) => {
