...
async fn handle_client(mut client_stream: TcpStream) -> Result<(), Error> {
    // Camuflagem inicial como WebSocket handshake
    let status = get_status();
    let fake_ws_handshake = format!(
    ...
    client_stream.write_all(fake_ws_handshake.as_bytes()).await?;

    // Aguarda um "ping" ou qualquer dado do cliente
    let result = timeout(Duration::from_secs(3), peek_stream(&mut client_stream)).await
        .unwrap_or_else(|_| Ok(String::new()));

    let mut addr_proxy = "0.0.0.0:22"; // default
